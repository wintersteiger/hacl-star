...
val gmac_alloc: unit -> StackInline gmac_state
  (requires (fun _ -> True))
  (ensures (fun h0 st h1 -> ~(contains h0 st.ram) /\
...
val gmac_init: st:gmac_state -> r:keyB{disjoint st.ram r /\ disjoint st.len r} -> Stack unit
  ...
  (ensures (fun h0 _ h1 -> valid_st h0 st /\ live h0 r /\ valid_st h1 st /\ live h1 r /\
    ...
    as_pure_st h1 st = gmac_init_spec (sel_word h0 r)))
let gmac_init st r =
  let h0 = ST.get() in
  let rv = encodeB r in
  st.ram.(0ul) <- rv;
  ...
  Seq.lemma_eq_intro (get_r h1 st) (encode (sel_word h0 r));
...
private val update_pmsg: pmsg:H128.t -> len_1:U32.t{valid_pmsg_ pmsg len_1} ->
  ...
  (ensures (fun h0 r h1 -> h0 == h1 /\ live h0 nmsg /\ 
    decode_pmsg r U32.(len_1 +^ len_2) = decode_pmsg pmsg len_1 @| sel_word h0 nmsg))
let update_pmsg pmsg len_1 nmsg len_2 = admit()
