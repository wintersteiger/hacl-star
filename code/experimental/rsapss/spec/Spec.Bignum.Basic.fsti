...
let blocks (x:size_pos) (m:size_pos): r:size_pos{x <= m * r} =
  (x - 1) / m + 1
...
val bn_cast_le:
    #n:size_pos
  -> m:size_pos{m <= n}
  -> b:bignum n{bn_v b < pow2 m}
  -> c:bignum m{bn_v c == bn_v b}
...
val bn_add_carry:
    #n:size_pos{n + 1 < max_size_t}
  -> #m:size_pos{m <= n}
  -> a:bignum n
  -> b:bignum m
  -> res:bignum (n + 1){bn_v res == bn_v a + bn_v b}

val bn_sub:
    #n:size_pos
  -> #m:size_pos //{m <= n}
  -> a:bignum n
  -> b:bignum m{bn_v a >= bn_v b}
  -> c:bignum n{bn_v c == bn_v a - bn_v b}

val bn_mul:
    #n:size_pos
  -> #m:size_pos{n + m < max_size_t}
  -> a:bignum n
  -> b:bignum m
  -> c:bignum (n + m){bn_v c == bn_v a * bn_v b}

val bn_get_bit:
    #n:size_pos
  -> b:bignum n
  -> i:size_nat{i < n}
  -> c:nat{(c == 0 \/ c == 1) /\ c == bn_v b / pow2 i % 2}
...
val bn_rshift:
    #n:size_pos
  -> b:bignum n
  -> i:size_pos{i < n /\ i % 64 == 0}
  -> c:bignum (n - i){bn_v c == bn_v b / pow2 i}
...
val bn_is_less:
  ...
  -> a:bignum n
  -> b:bignum m
  -> c:bool{if c then bn_v a < bn_v b else bn_v a >= bn_v b}
...
val bn_to_bytes_be:
    #bBits:size_pos
  -> n:bignum bBits
  -> b:lbytes (blocks bBits 8){bn_v n == nat_from_bytes_be b}

val bn_pow2_r_mod:
    #nBits:size_pos
  -> n:bignum nBits{bn_v n > 0}
  -> r:size_pos
  -> c:bignum nBits{bn_v c == pow2 r % bn_v n}
