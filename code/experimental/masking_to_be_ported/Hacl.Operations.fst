...
val setallbuf_aux: #t:Type -> b:buffer t -> z:t -> len:U32.t -> ctr:U32.t -> STL unit
  (requires (fun h ->
            (live h b)
            /\ (U32.v len = Seq.length (FB.as_seq h b))
            /\ (U32.v ctr <= U32.v len)
...
let rec setallbuf_aux #t b z len ctr =
  if (len -^ ctr) =^ 0ul then ()
  else begin
    FB.upd b ctr z;
    setallbuf_aux #t b z len (ctr +^ 1ul) end

val setallbuf: #t:Type -> b:buffer t -> z:t -> len:U32.t -> STL unit
  (requires (fun h ->
            (live h b)
            /\ (U32.v len = Seq.length (FB.as_seq h b))))
  (ensures  (fun h0 _ h1 ->
...
let setallbuf #t b z len = setallbuf_aux #t b z len 0ul
