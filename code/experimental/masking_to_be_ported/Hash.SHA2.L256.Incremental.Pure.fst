...
let iseq32 = Seq.seq i32
let ibytes = Seq.seq i8

(* Has to be implemented and relocated *)
assume val be_8ibytes_of_int: int -> Tot (b:ibytes{Seq.length b = 8})
assume val be_ibytes_of_iseq32: a:iseq32 -> Tot (b:ibytes{Seq.length b = 4 ** Seq.length a})
assume val be_iseq32_of_ibytes: a:ibytes -> Tot (b:iseq32{4 ** (Seq.length b) = Seq.length a})
...
(*  Section 5.1.1: Compute the pad length (without the last 8 ibytes) *)
...
let pad (input:ibytes) : Tot (block:ibytes) =

  (* Compute the padding length *)
  let padlen = pad_length (Seq.length input) in

  (* Generate the padding (without the last 8 ibytes) *)
  let padding = Seq.create padlen 0uy in

  (* Set the first bit of the padding to be a '1' *)
  let padding = Seq.upd padding 0 0x80uy in

  (* Compute the MD length to be encoded as ibytes *)
  let encodedlen = be_8ibytes_of_int ((Seq.length input) ** 8) in
...
let init () : Tot (ihash:iseq32{Seq.length ihash = 8}) =
  admit(); // Required for verification timeout
...
let rec ws (block:iseq32{Seq.length block = 16}) (t:nat{t < Seq.length block}) : Tot i32 =
...
let rec update_inner (whash:iseq32{Seq.length whash = 8}) (block:iseq32{Seq.length block = 16}) (t1:i32) (t2:i32) (t:nat{t <= 64}) 
  : Tot (ohash:iseq32{Seq.length ohash = 8}) =
...
let update_1 (whash:iseq32{Seq.length whash = 8}) (block:ibytes{Seq.length block = blocksize}) : Tot (ohash:iseq32{Seq.length ohash = 8}) =

  // Invariant: update_1 l l[i-1].hash l[i-1].data == l[i].hash
  ...
  (* Linearize *)
  let block32 = be_iseq32_of_ibytes block in
  
  (* Step 3 : Perform logical operations on the working variables *)
  let oH = update_inner whash block32 0ul 0ul 0 in
...
(* Finish function *)
let finish (whash:iseq32{Seq.length whash = 8}) (last:ibytes) : GTot (hash:ibytes{Seq.length hash = hashsize}) =

  (* Pad the last block of data *)
  let blocks = pad last in

  (* Process the final block of data *)
  let hash = 
    match blocks with
    | Some block1, Some block2 -> begin update_1 (update_1 whash block1) block2 end
    | Some block1, _ -> begin update_1 whash block1 end
  in
  
  (* Flatten the working hash and return the final value *)
  be_ibytes_of_iseq32 hash
