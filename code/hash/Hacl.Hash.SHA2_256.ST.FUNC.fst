...
noeq type stateful 'b = 
| MkStateful: 
      spec:Spec.stateful 'b ->
      impl:(s:state -> Stack 'b
...
unfold let read (k:key) (x:u32{v x < seqlen k}) : 
       r:stateful (Spec.value k){forall st. r.spec st == Spec.read k (v x) st} =
  MkStateful (Spec.read k (v x)) 
	     (fun st -> let b = get_buf st k in 
		      b.(x))

unfold let write (k:key) (x:u32{v x < seqlen k}) (vl:value k) : 
       r: stateful unit{forall st. r.spec st == Spec.write k (v x) vl st} =
  MkStateful (Spec.write k (v x) vl) 
	     (fun st -> let b = get_buf st k in
		      b.(x) <- vl)
		      
unfold let bind (f:stateful 'b) (g:x:'b -> stateful 'c) : 
       r: stateful 'c{forall st. r.spec st == Spec.bind f.spec (fun x -> (g x).spec) st} =
  MkStateful (Spec.bind f.spec (fun x -> (g x).spec))
	     (fun st -> let r = f.impl st in
		      (g r).impl st)

let return (x:'b): r:stateful 'b{forall st. r.spec st == Spec.return x st} = 
  MkStateful (Spec.return x)
	     (fun st -> x)

let apply_write (k:key) 
    (spec: Spec.Lib.seq1_st (value k) (seqlen k) 'b) 
    (f:b:buffer (value k){length b = seqlen k} -> Stack 'b 
	      (requires (fun h -> live h b))
	      (ensures (fun h0 r h1 -> live h1 b /\ modifies_1 b h0 h1 /\ 
		       spec (as_seq h0 b) == (r, as_seq h1 b))))
   : r: stateful 'b{forall st. r.spec st == Spec.apply_write k spec st} =
   MkStateful (Spec.apply_write k spec) 
	      (fun st -> f (get_buf st k))

assume val iteri: min:u32 -> max:u32{v min <= v max} -> 
	   f:(x:u32{v x >= v min /\ v x < v max} -> stateful unit) ->
	   r:stateful unit{forall st. r.spec st = Spec.iteri (v min) (v max) 
					      (fun x -> (f (UInt32.uint_to_t x)).spec) st}
	   
(* BEGIN IMPL *)

let setup_k : stateful unit = 
  MkStateful Spec.setup_k
  (apply_write K 
  (Spec.Lib.seq1_upd64 #u32
...
let setup_h0 : stateful unit = 
  MkStateful Spec.setup_h_0
    (fun st -> hupd_8 (get_h0 st)
      (0x6a09e667ul) (0xbb67ae85ul) (0x3c6ef372ul) (0xa54ff53aul)
      (0x510e527ful) (0x9b05688cul) (0x1f83d9abul) (0x5be0cd19ul))

let setup_ws0 : stateful unit =
  MkStateful Spec.setup_ws0
  (iteri 0ul 16ul 
  (fun i ->
     x <-- read Data i ;
     let y:u32 = x in
