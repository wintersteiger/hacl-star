module Hacl.Symmetric.Poly1305.Spec
...
let rec little_endian_null len =
  if len = 0 then ()
    ...
    assert (little_endian (Seq.create len (uint8_to_sint8 0uy)) ==
      0 + pow2 8 * little_endian (Seq.slice (Seq.create len (uint8_to_sint8 0uy)) 1 len));
...
let rec lemma_little_endian_is_bounded b =
  if Seq.length b = 0 then ()
    ...
    lemma_euclidean_division (H8.v (Seq.index b 0)) (little_endian s);
...
let clamp r =
  ...
  let r = fix r  3  mask_15 in // 0000****
  let r = fix r  7  mask_15 in
  let r = fix r 11  mask_15 in
  let r = fix r 15  mask_15 in
  let r = fix r  4 mask_252 in // ******00
  let r = fix r  8 mask_252 in
  let r = fix r 12 mask_252 in
  little_endian r
