...
open Hacl.Symmetric.Poly1305.Spec
...
let zeroB a =
  ...
  a.(4ul) <- zero;
  let h = ST.get() in
  Hacl.Symmetric.Poly1305.Bigint.eval_null h a norm_length
...
private val mk_mask: nbits:FStar.UInt32.t{FStar.UInt32.v nbits < 64} ->
  Tot (z:H64.t{v z = pow2 (FStar.UInt32.v nbits) - 1})
let mk_mask nbits =
...
let lemma_toField_1 (b:elemB) (s:wordB_16{disjoint b s}) h n0 n1 n2 n3 : Lemma
  (requires (let open Hacl.UInt8 in
    live h b /\ live h s
    /\ H64.v n0 = v (get h s 0) + pow2 8 * v (get h s 1) + pow2 16 * v (get h s 2) + pow2 24 * v (get h s 3)
    /\ H64.v n1 = v (get h s 4) + pow2 8 * v (get h s 5) + pow2 16 * v (get h s 6) + pow2 24 * v (get h s 7)
    /\ H64.v n2 = v (get h s 8) + pow2 8 * v (get h s 9) + pow2 16 * v (get h s 10) + pow2 24 * v (get h s 11)
    /\ H64.v n3 = v (get h s 12) + pow2 8 * v (get h s 13) + pow2 16 * v (get h s 14) + pow2 24 * v (get h s 15)))
...
let toField b s =
  ...
  let n0 = (Utils.uint32_of_bytes s0) in
  let n1 = (Utils.uint32_of_bytes s1) in
  let n2 = (Utils.uint32_of_bytes s2) in
  let n3 = (Utils.uint32_of_bytes s3) in
...
val lemma_toField_plus_2_128_1: unit -> Lemma (v (uint64_to_sint64 1uL <<^ 24ul) = pow2 24)
...
val upd_wordB_16: b:wordB_16 -> s0:H8.t -> s1:H8.t -> s2:H8.t -> s3:H8.t -> s4:H8.t -> s5:H8.t ->
  s6:H8.t -> s7:H8.t -> s8:H8.t -> s9:H8.t -> s10:H8.t -> s11:H8.t -> s12:H8.t -> s13:H8.t ->
  s14:H8.t -> s15:H8.t -> Stack unit
    (requires (fun h -> live h b))
    (ensures  (fun h0 _ h1 -> live h1 b /\ modifies_1 b h0 h1))
...
let trunc1305 a b =
  ...
  let b0 = sint64_to_sint8 a0 in
  let b1 = sint64_to_sint8 (a0 >>^ 8ul) in
  let b2 = sint64_to_sint8 (a0 >>^ 16ul) in
  let b3 = sint64_to_sint8 ((a0 >>^ 24ul) +%^ (a1 <<^ 2ul)) in
  let b4 = sint64_to_sint8 (a1 >>^ 6ul) in
  let b5 = sint64_to_sint8 (a1 >>^ 14ul) in
  let b6 = sint64_to_sint8 ((a1 >>^ 22ul) +%^ (a2 <<^ 4ul)) in
  let b7 = sint64_to_sint8 (a2 >>^ 4ul) in
  let b8 = sint64_to_sint8 (a2 >>^ 12ul) in
  let b9 = sint64_to_sint8 ((a2 >>^ 20ul) +%^ (a3 <<^ 6ul)) in
  let b10 = sint64_to_sint8 (a3 >>^ 2ul) in
  let b11 = sint64_to_sint8 (a3 >>^ 10ul) in
  let b12 = sint64_to_sint8 (a3 >>^ 18ul) in
  let b13 = sint64_to_sint8 a4 in
  let b14 = sint64_to_sint8 (a4 >>^ 8ul) in
  let b15 = sint64_to_sint8 (a4 >>^ 16ul) in
...
let clamp r =
  ...
  fix r  3ul  mask_15; // 0000****
  fix r  7ul  mask_15;
  fix r 11ul  mask_15;
  fix r 15ul  mask_15;
  fix r  4ul mask_252; // ******00
  fix r  8ul mask_252;
  fix r 12ul mask_252
...
let poly1305_init r s key =
  ...
  let r_16 = create (uint8_to_sint8 0uy) 16ul in
...
val poly1305_update:
  ...
  r:elemB{disjoint msg r /\ disjoint acc r} -> Stack unit //log_t
...
let poly1305_update (* log *) msgB acc r =
  let h0 = ST.get () in
  push_frame();
  let block = create (uint64_to_sint64 0uL) nlength in // TODO: pass buffer, don't create one
...
val poly1305_loop: (* current_log:log_t ->  *)
  msg:bytes -> acc:elemB{disjoint msg acc} ->
  r:elemB{disjoint msg r /\ disjoint acc r} ->
  ctr:u32{length msg >= 16 * w ctr} ->
  Stack unit (* log_t *)
  (requires (fun h -> live h msg /\ norm h acc /\ norm h r))// /\
      (* (mac_log ==> *)
...
let add_word a b =
  let carry = 0ul in
  let a04:H64.t = let (x:H32.t) = Utils.uint32_of_bytes a in sint32_to_sint64 x  in
  let a48:H64.t = let (x:H32.t) = Utils.uint32_of_bytes (offset a 4ul) in sint32_to_sint64 x in
  let a812:H64.t = let (x:H32.t) = Utils.uint32_of_bytes (offset a 8ul) in sint32_to_sint64 x in
  let a1216:H64.t = let (x:H32.t) = Utils.uint32_of_bytes (offset a 12ul) in sint32_to_sint64 x in
  let b04:H64.t = let (x:H32.t) = Utils.uint32_of_bytes b in sint32_to_sint64 x  in
  let b48:H64.t = let (x:H32.t) = Utils.uint32_of_bytes (offset b 4ul) in sint32_to_sint64 x in
  let b812:H64.t = let (x:H32.t) = Utils.uint32_of_bytes (offset b 8ul) in sint32_to_sint64 x in
  let b1216:H64.t = let (x:H32.t) = Utils.uint32_of_bytes (offset b 12ul) in sint32_to_sint64 x in
  let open Hacl.UInt64 in
  let z0 = a04 +^ b04 in
  let z1 = a48 +^ b48 +^ (z0 >>^ 32ul) in
  let z2 = a812 +^ b812 +^ (z1 >>^ 32ul) in
  let z3 = a1216 +^ b1216 +^ (z2 >>^ 32ul) in
  Utils.bytes_of_uint32 (Buffer.sub a 0ul 4ul) (sint64_to_sint32 z0);
  Utils.bytes_of_uint32 (Buffer.sub a 4ul 4ul) (sint64_to_sint32 z1);
  Utils.bytes_of_uint32 (Buffer.sub a 8ul 4ul) (sint64_to_sint32 z2);
  Utils.bytes_of_uint32 (Buffer.sub a 12ul 4ul) (sint64_to_sint32 z3);
  admit ()
...
let poly1305_mac tag msg len key =
  ...
  let s   = create (uint8_to_sint8 0uy) 16ul in
