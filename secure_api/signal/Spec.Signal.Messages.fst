...
let plain_bytes = b:bytes{length b + 16 <= max_size_t /\ cipherlen (length b) + 140 <= max_size_t}
let cipher_bytes = b:bytes{length b + 140 <= max_size_t /\ length b > 0 /\ length b % 16 == 0}
...
let mac_whisper_msg
  ...
  (whisper_msg:bytes{length whisper_msg + size_mac_whisper_msg_extra_info <= max_size_t})
  : Tot (lbytes 8) =
