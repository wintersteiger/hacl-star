...
val initiate':
  ...
  Stack unit
    (requires (fun h -> live h new_root_key /\ live h our_identity_priv_key
		   ...
		   /\ live h their_signed_pub_key /\ live h new_chain_key
		   /\ live h their_onetime_pub_key
		   /\ disjoint new_root_key new_chain_key
...
let initiate' new_root_key new_chain_key our_identity_priv_key our_onetime_priv_key
  ...
  initiate root_key our_identity_priv_key our_onetime_priv_key their_identity_pub_key
...
val fill_message_keys:
  ...
  -> chain_key:privkey_p
  -> Stack unit
    (requires (fun h -> live h mac /\ live h new_chain_key /\ live h chain_key /\
      disjoint mac new_chain_key /\ disjoint mac chain_key /\
      disjoint new_chain_key chain_key
    ))
    (ensures (fun h0 _ h1 -> modifies2 new_chain_key mac h0 h1 /\
      begin let (expected_mac, expected_chain_key) = Spec.Signal.Core.fill_message_keys
	h0.[chain_key]
      in
      expected_mac == h1.[mac] /\ expected_chain_key == h1.[new_chain_key]
      end
    ))

let fill_message_keys mac new_chain_key chain_key =
  (**) recall_contents #uint8 #(size 1) const_one Spec.Signal.Messages.one;
  hmac #(Ghost.hide 1) mac chain_key (size 1) const_one;
...
noextract val serialize_and_add_mac_spec:
     #plen:(Ghost.erased size_nat){
       ...
       Spec.Signal.Crypto.cipherlen (Ghost.reveal plen) + 140 <= max_size_t
     }
