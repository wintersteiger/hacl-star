...
// we also have FStar's bytes and lbytes 
...
// TODO? I'd rather see Spec.Hash use the simpler style above than
// write an inductive proof.

val lemma_hash_spec: a:alg13 -> b: hashable a -> Lemma (hash a b = spec a b)


(* ONE-SHOT IMPLEMENTATION; no need for an explicit bound since len <= 2^32 *)
...
// TODO lowering the commented-out code in Hashing.fst 

(* IMPLEMENTATION DETAILS supporting agile incremental hash, hmac, etc *)
...
val update_last:
  ...
  total_len: UInt32.t {Buffer.length last = v total_len % blockLength a} ->
  Stack unit 
  (requires fun h0 -> 
  ...
  (ensures fun h0 () h1 -> 
    ...
    Buffer.modifies_1 #(if a = SHA256 then UInt32.t else UInt64.t) st h0 h1 /\ 
    ( let l1 = v total_len % blockLength a in
      let l0 = v total_len - l1 in 
      forall (prior: lbseq l0). 
      ( as_acc h0 st = hash2 acc0 prior ==>
        finish (as_acc h1 st) = spec a (Seq.append prior (Buffer.as_seq h0 last)) )))

// for HSL we'll need a variant that copies, finalizes, and extracts.
