...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    int numchars = get_line(childfd, buf, sizeof(buf));
    printf("%s", buf);
    sscanf(buf, "%s %s %s\n", method, uri, version);
