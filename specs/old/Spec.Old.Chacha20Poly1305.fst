module Spec.Old.Chacha20Poly1305
...
let poly1305_aad (k:key) (n:nonce) (m:bytes) (aad:bytes) =
    let b0 = Spec.Old.Chacha20.chacha20_block k n 0 in
    ...
    let mac = Spec.Old.Poly1305.poly1305 to_mac mackey in
    mac

#set-options "--initial_fuel 0 --max_fuel 0 --z3rlimit 30"
val aead_chacha20_poly1305_encrypt: k:key -> n:nonce ->
    m:bytes -> aad:bytes{(length m + length aad) / Spec.Old.Chacha20.blocklen < pow2 32} ->
Tot (lbytes (length m) * Spec.Old.Poly1305.tag)
let aead_chacha20_poly1305_encrypt k n m aad =
    let cipher = Spec.Old.Chacha20.chacha20_encrypt_bytes k n 1 m in
    let mac = poly1305_aad k n cipher aad in
    (cipher, mac)

val aead_chacha20_poly1305_decrypt: k:key -> n:nonce ->
    c:bytes -> tag:lbytes 16 ->  aad:bytes{(length c + length aad) / Spec.Old.Chacha20.blocklen < pow2 32} ->
    Tot (option (lbytes (length c)))
let aead_chacha20_poly1305_decrypt k n c mac aad =
    let xmac = poly1305_aad k n c aad in
    if xmac = mac then
       let plain = Spec.Old.Chacha20.chacha20_encrypt_bytes k n 1 c in
       Some plain
    else None
