...
let setup (k:key) (n:nonce) (c:counter): Tot state =
  let constants : vec = Seq.Old.Create.create_4 c0 c1 c2 c3 in
  ...
  Seq.Old.Create.create_4 constants key_part_1 key_part_2 nonce
...
let chacha20_ctx: Spec.Old.CTR.block_cipher_ctx =
    let open Spec.Old.CTR in
    {
    keylen = keylen;
    blocklen = blocklen;
    noncelen = noncelen;
...
let chacha20_cipher: Spec.Old.CTR.block_cipher chacha20_ctx = chacha20_block

let chacha20_encrypt_bytes key nonce counter m =
    Spec.Old.CTR.counter_mode chacha20_ctx chacha20_cipher key nonce counter m
