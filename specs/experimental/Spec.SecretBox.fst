...
#set-options "--initial_fuel 2 --max_fuel 2 --z3rlimit 300"
val lemma_ciphertext_length: m:nat -> n:nat -> Lemma
  (requires (n = m + 16 /\ m / Spec.Salsa20.blocklen < pow2 32))
  (ensures ((n = m + 16 /\ m / Spec.Salsa20.blocklen < pow2 32) ==> ((n - 16) / Spec.Salsa20.blocklen < pow2 32)))
let lemma_ciphertext_length m n =
  ()
...
let secretbox_easy m k n =
    ...
    let c = (mac @| cipher) in
    assert(length c >= 16);
    assert((length c) - 16 / Spec.Salsa20.blocklen < pow2 32);
    c

//(fun n -> n >= 16 && (n - 16) / Spec.Salsa20.blocklen < pow2 32)
